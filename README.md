...
Sanity is not going to use a traditional inheritence model like Java does (see
[Proxies](#proxies) and [Mixins](#mixins) for Sanity's solution to that particular issue).
However, it is possible to use this system with a Java-like inheritence model, it simply
requires decoupling between superclass and subclass construction.
